    ...
    <div class="relative pb-24 pt-2 bg-zinc-200">
        <div class="container mx-auto">
            <ul class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-4">
                <ArticleItem
                    v-for="(article, index) in articleList.filter((article) =>
                        hideDeleted ? !article.is_deleted : true
                    )"
                    :key="article.aid"
                    :index="index + 1"
                    :title="article.title"
...
const syncedIdList = reactive<String[]>([]);
...
async function loadSyncedIds() {
    const host = "/mapi";
    const url = "m-system/sysweixinarticle/idList";
    const result = await $fetch<{ code: Number; data: String[] }>(`${host}/${url}`, {
        method: "GET",
